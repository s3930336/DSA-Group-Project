...
class Robot {
	 ...
	 */
	private int[][] map = new int[Maze.MAX_ROW*2][Maze.MAX_COL*2];
	private Coordinate robotPosition = new Coordinate(Maze.MAX_ROW, Maze.MAX_COL);
	
	/* Navigate method
	 * The navigate method of the robot will repeatedly attempt to find the exit in the maze, one step at a time,
	...
	private String directionToGo() {
		...
		//Once the 4 directions has been arranged in the heap, we can pop the heap to get the direction we want to move
		DirectionInfo result;
		while(!heap.isEmpty()) {
			result = heap.pop();
			if(result.isWall()) continue;		//If the direction is a wall, we ignore and pop the next direction
			else return result.direction();
		}
 ...
 * This MinHeapDI class holds objects of the DirectionInfo class and put them into a minimum heap, 
 * where the root of a tree is always the minimum object
 * Public operations:
 * 					buildHeap(): build MinHeap from an array of type DirectionInfo
 * 					push(): add a new DirectionInfo object to the heap
 * 					pop(): return the DirectionInfo object at the root of the MinHeap and delete the object from the heap
 * 
 * Acknowledgement: 
 * 	Some elements of this class has been modified from the example solution
 *  	from week 4 module, Problem 3 (from the course public GitHub)
 */ 
class MinHeapDI {
	...
	private void heapifyUp(int nodeIndex) {
		...
		//If the element at the index is smaller than its parent, swap, then recursively call heapifyUp on the parentIndex after swap
		...
		//If no swap happens, then the element at the index must be at the right position
		return;
	}
...
/*
 * DirectionInfo Class
 * The class is to create an object that corresponds to each of the four directions that the robot can move to at each space.
 * This object is one of the 4 directions and contains the number of visits the robot has made in that space.
 * The number of visit can be -1 to represent a invalid space (a "wall")
 * The directional priority of this class ("smaller" direction) is defined as "UP" < "RIGHT" < "DOWN" < "LEFT"
 */
class DirectionInfo{
	...
	/*=========================================
	 * Comparison Methods
	 * The comparison methods of the class will determine how the direction is chosen at each step
	...
	public boolean isSmaller(DirectionInfo other) {
		...
		if (this.visitNum > other.visitNum) return false;
		else if (this.visitNum < other.visitNum) return true;
		else return this.directionalPriority < other.directionalPriority;
	}
